...
void szescian6()
{
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	{
		...
		//parametry wierzcholkow okreœlaj¹cych po³o¿enie górnej (czerwonej œciany)
...
{
	float kat = 0;
	//rysowanie prawego pó³³uku (w stosunku do pocz¹tkowego widoku) figury od 0 stopni do 90 stopni
	for (kat = 0; kat < GL_PI / 2 + GL_PI / 192; kat += GL_PI / 192)
	{
		rysujSciane(80 - (40 + cos(kat) * 20), 2, 40, 40 + cos(kat) * 20, 0 + sin(kat) * 20, 0);
	}

	//rysowanie lewego pó³³uku (w stosunku do pocz¹tkowego widoku) figury od 180 stopni do 90 stopni
...
//o zadanej ilosci cegiel w krótszej wartswie, oraz zadanej iloœci warstw
